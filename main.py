...
class DataPipeline:
    ...
    def close_spider(self, spider):
        ...
        with pymongo.MongoClient('mongodb+srv://<user>:<password>@mongodb.dtj8cur.mongodb.net/'
                                 '?retryWrites=true&w=majority&appName=mongoDB') as client:
            db = client["mongodb"]

            db.insert_one({"quotes": self.quotes})
            db.insert_one({"authors": self.authors})
...
class QuotesSpider(scrapy.Spider):
    ...
    custom_settings = {'ITEM_PIPELINES': {DataPipeline: 400}}
